		...
		if ifirst[j] != -1:
			...
			if len(avspeed[j])>4:
				for i in range(len(avspeed[j])-1):
					...
					formula=1.258*10**(-4)*((length[j][0])**2)*(((acspeed[j][i+1]+acspeed[j][i])*1.852/2)**3)*(interval[j][i+1]-interval[j][i])*c*(12)*0.9*0.98/3600
					load=((acspeed[j][i+1]+acspeed[j][i])*1.852/2/10.76)**3
					if load >=0.195:
