# -*- coding: utf-8 -*-
"""
Created on Tue Nov 12 20:28:08 2019
...
# inputfile = r'E:\work\calculate emission\data2\output_20181201.csv'
	...
	ilon         = [0] * nr_ships
	ilat         = [0] * nr_ships
	cross_lon    = [0] * nr_ships # false
	cross_lat    = [0] * nr_ships # false
	per_emi      = [0] * nr_ships
	emi          = [0] * nr_ships
	bi           = [0] * nr_ships
	...
	for shipcode in mmsi:
		...
		for j in range(nr_ships):
			if shipcode == shiplist[j]:
				...
				if float(lons[i]) > lim_lon:
					cross_lon[j] = 1   # true
				if float(lats[i]) < lim_lat:
					cross_lat[j] = 1   # true
		...
		if ifirst[j] != -1:
			...
			if len(avspeed[j])>4:
				for i in range(len(avspeed[j])-1):
					...
					formula=4.869*10**(-5)*((length[j][0])**2)*(((acspeed[j][i+1]+acspeed[j][i])*1.852/2)**3)*(interval[j][i+1]-interval[j][i])*c*(15.7*0.044+11.6*0.2+10.6*0.756)*0.9*0.98/3600
					load=((acspeed[j][i+1]+acspeed[j][i])*1.852/2/19.8)**3
					if load >=0.195:
						emission.append(formula)
					elif 0.185<= load < 0.195:
						emission.append(formula*1.01)
					elif 0.175<= load < 0.185:
						emission.append(formula*1.02)
					elif 0.165<= load < 0.175:
						emission.append(formula*1.03)
					elif 0.155<= load < 0.165:
						emission.append(formula*1.05)
						...
						emission.append(formula*1.14)
					elif 0.105<= load < 0.115:
						emission.append(formula*1.17)
					elif 0.095<= load < 0.105:
						emission.append(formula*1.22)
					elif 0.085<= load < 0.095:
						emission.append(formula*1.27)
					elif 0.075<= load < 0.085:
						emission.append(formula*1.35)
					elif 0.065<= load < 0.075:
						emission.append(formula*1.45)
					elif 0.055<= load < 0.065:
						emission.append(formula*1.6)
					elif 0.045<= load < 0.055:
						emission.append(formula*1.83)
					elif 0.035<= load < 0.045:
						emission.append(formula*2.21)
					elif 0.025<= load < 0.035:
						emission.append(formula*2.92)
					elif 0.015<= load < 0.025:
						emission.append(formula*4.63)
					elif 0< load < 0.015:
						emission.append(formula*11.47)
					if interval[j][i+1]-interval[j][i]>600: # if ship interval>10 min, ship will remove other place by definition
						if load >=0.195:
							bias.append(formula)
						elif 0.185<= load <0.195:
							bias.append(formula*1.01)
						elif 0.175<= load <0.185:
							bias.append(formula*1.02)
						elif 0.165<= load <0.175:
							bias.append(formula*1.03)
						elif 0.155<= load <0.165:
							bias.append(formula*1.05)
						elif 0.145<= load <0.155:
							...
							bias.append(formula*1.14)
						elif 0.105<= load <0.115:
							bias.append(formula*1.17)
						elif 0.095<= load <0.105:
							bias.append(formula*1.22)
						elif 0.085<= load <0.095:
							bias.append(formula*1.27)
						elif 0.075<= load <0.085:
							bias.append(formula*1.35)
						elif 0.065<= load <0.075:
							bias.append(formula*1.45)
						elif 0.055<= load <0.065:
							bias.append(formula*1.6)
						elif 0.045<= load <0.055:
							bias.append(formula*1.83)
						elif 0.035<= load <0.045:
							bias.append(formula*2.21)
						elif 0.025<= load <0.035:
							bias.append(formula*2.92)
						elif 0.015<= load <0.025:
							bias.append(formula*4.63)
						elif 0< load <0.015:
							bias.append(formula*11.47)
				bi[j]=sum(bias)		
