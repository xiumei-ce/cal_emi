	...
	with open(inputfile,'r') as csvfile:
		...
		for rows in readCSV:
			...
			if 0 <= speed_ship <= 30 and 0 < ilength < 400 and 118.6< longitude <119.0 and 31.9< latitude <32.3:
				if 10<= ship_type <31 or 33 < ship_type < 52 or 52 < ship_type < 55 or 55 < ship_type <60 or 89< ship_type <132:
		...
		if ifirst[j] != -1:
			...
			if len(avspeed[j])>4:
				for i in range(len(avspeed[j])-1):
					...
					formula=6.602*10**(-5)*((length[j][0])**2)*(((acspeed[j][i+1]+acspeed[j][i])*1.852/2)**3)*(interval[j][i+1]-interval[j][i])*c*(16*0.051+11.9*0.355+10.6*0.594)*0.9*0.98/3600
					load=((acspeed[j][i+1]+acspeed[j][i])*1.852/2/16.4)**3
					if load>=0.195:
