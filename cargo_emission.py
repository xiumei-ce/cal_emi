...
# inputfile = r'E:\work\emission\data1\Oct\output_20181025.csv'
		...
		if ifirst[j] != -1:
			...
			if len(acspeed[j])>4:
				for i in range(len(acspeed[j])-1):
					...
					formula=4.755*10**(-5)*((length[j][0])**2)*(((acspeed[j][i+1]+acspeed[j][i])*1.852/2)**3)*(interval[j][i+1]-interval[j][i])*c*(16.2*0.496+11.5*0.404+10.6*0.1)*0.9*0.98/3600
					load=((acspeed[j][i+1]+acspeed[j][i])*1.852/2/15.33)**3
					if load >=0.195:
...
##### 建立文件########
# workbook = xlsxwriter.Workbook("NOx.xlsx")
# 可以制定表的名字
# worksheet = workbook.add_worksheet('text')
# worksheet = workbook.add_worksheet()
#worksheet.write_row('A1',headings) #按行插入
# worksheet.write_column('B1',nemission) #按列插入
# workbook.close()
#######################

#将数据存储到表格中
write_no(1,nemission)
