...
# inputfile = r'D:\work\emission\data\output_20190331.csv'
		...
		if ifirst[j] != -1:
			...
			if len(avspeed[j])>4:
				for i in range(len(avspeed[j])-1):
					...
					formula=1.258*10**(-4)*((length[j][0])**2)*(((acspeed[j][i+1]+acspeed[j][i])*1.852/2)**3)*(interval[j][i+1]-interval[j][i])*c*(11.5)*0.56*0.98/3600
